...
def normalize(x):
    """Translate and scale a point set to make it have zero mean and unit
    variance. Return the point set after normalization, its original centroid
    and scale.
    """
    centroid = x.mean(0)
    x = x - centroid
...
def L2_distance(model, scene, scale):
    """Compute the L2 distance between the two Gaussian mixture densities
    constructed from a moving 'model' point set and a fixed 'scene' point set
    at a given 'scale'. The term that only involves the fixed 'scene' is
    excluded from the returned distance.  The gradient with respect to the
    'model' is calculated and returned as well.
    """
...
def correlation(model, scene, scale):
    """Compute the normalized correlation between the two Gaussian mixture
    densities constructed from a moving 'model' point set and a fixed 'scene'
    point set at a given 'scale'. The term that only involves the fixed 'scene'
    is excluded from the returned distance.  The gradient with respect to the
    'model' is calculated and returned as well.
    """
    f1, g1 = gauss_transform(model, model, scale)
    f2, g2 = gauss_transform(model, scene, scale)
...
def compute_GRBF(ctrl_pts, landmarks, sigma):
    ...
    K = array([kernel_func(norm(x-y), sigma)
               for x in ctrl_pts for y in ctrl_pts])
    K = K.reshape(len(ctrl_pts), len(ctrl_pts))
    U = array([kernel_func(norm(x-y), sigma)
               for x in landmarks for y in ctrl_pts])
...
def compute_TPS_K(ctrl_pts, landmarks=None, _lambda=0):
    ...
    n, d = ctrl_pts.shape
    K = [kernel_func[d-2](norm(ctrl_pts[i]-ctrl_pts[j]), _lambda)
         for i in arange(n) for j in arange(n)]
    K = array(K).reshape(n, n)
    if landmarks is not None:
        m, d = landmarks.shape  # assert (d, d) equal
        U = [kernel_func[d-2](norm(landmarks[i]-ctrl_pts[j]), _lambda)
             for i in arange(m) for j in arange(n)]
...
def prepare_TPS_basis(landmarks, ctrl_pts):
    """Return the basis for performing TPS transforms and the kernel matrix
    for computing the bending energy."""
    m, d = landmarks.shape
    n, d = ctrl_pts.shape
...
def obj_TPS(dist_func, param, basis, kernel, scene, scale, _lambda):
    """
...
def run_multi_level(model, scene, ctrl_pts, level, scales, lambdas, iters):
    ...
    for i in range(level):
        x = fmin_l_bfgs_b(
                obj_L2_TPS, x0, None,
                args=(basis, kernel, scene, scales[i], lambdas[i]),
                maxfun=iters[i])
        x0 = x[0]
